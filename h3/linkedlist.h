...
/* __________________
  ...
  \__________________/
    ...
    Error Handling: Whenever a function is given invalid parameter values 
        the message "!-- ERROR : PANIC in LINKEDLIST!!" and maybe an
explanation
...
public:
  ...
  LinkedList(const T& x, LinkedList<T>* p) 
    ...
    // YOU are expected to implement function clear() to de-allocate the
    // list
		// --INLINE
  ~LinkedList() {}

    // Purpose: performs a deep copy of the data from rhs into this linked
    // list
    // Parameters: rhs is linked list to be copied
    // Returns: *this
    // Postconditions: this list contains same data values (in the same
    // order)
  ...
  LinkedList<T>* getAtPtr(int i);
    ...
    // Postconditions: all dynamically allocated memory for nodes
    // deallocated
  void clear();
